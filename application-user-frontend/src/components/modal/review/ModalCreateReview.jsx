...
import { getReviewFeelings, parseReviewMessage } from '../../../utils/functionUtils';
...
function ModalCreateReview({ open, handleClose, movieId, onSetPage }) {
    ...
    useEffect(() => {
        setSelectedFeelings([]);
        setFeelings(getReviewFeelings(rating));
    }, [rating])
    ...
    const onSubmit = data => {
        ...
        createReview(formData)
            ...
            .then(() => {
                ...
                // Giải phóng tất cả các URL tạm thời sau khi tải lên thành công
                imageURLs.forEach(url => URL.revokeObjectURL(url));

                // Xóa hoặc reset các file và URLs sau khi tải lên thành công
                setFiles([]);
                setImageURLs([]);
            })
            .catch((error) => {
